...
export default function GPXQuizPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[var(--on-surface)]">GPX — Quiz</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {GPX_COURSE_MODULES.map((mod) => (
          <Link key={mod.id} href={`/gpx/quiz/${mod.id}`} className="rounded-2xl border border-[var(--outline)] bg-[var(--surface)] p-5 hover:border-[#1147D9]/40 hover:shadow-md transition-all group">
