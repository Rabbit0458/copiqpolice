...
export default function ProfilPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      setUser(data.user)
      const { data: p } = await (supabase as any).from("profiles").select("*").eq("id", data.user.id).maybeSingle()
      setProfile(p)
      setLoading(false)
    })
  }, [])
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl border border-[var(--outline)] bg-[var(--surface)] p-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="font-semibold text-[var(--on-surface)] text-lg">{profile?.full_name || user?.email?.split("@")[0]}</p>
