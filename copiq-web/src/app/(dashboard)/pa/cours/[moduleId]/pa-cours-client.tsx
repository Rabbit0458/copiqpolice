...
export default function PACoursClient() {
  ...
  if (!mod || mod.category !== "pa") return <div className="flex items-center justify-center h-64"><p className="text-[var(--on-surface-muted)]">Module introuvable</p></div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-sm text-[var(--on-surface-muted)] mb-6">
        ...
        <Link href="/pa/scolarite" className="hover:text-[var(--on-surface)]">PA Scolarité</Link>
        <ChevronRight size={14} />
      ...
      <div className="mt-6 flex items-center justify-between">
        <Link href="/pa/scolarite" className="px-4 py-2.5 rounded-xl border border-[var(--outline)] text-sm text-[var(--on-surface-muted)] hover:text-[var(--on-surface)] transition-all">← Retour</Link>
        <Link href="/pa/quiz" className="px-4 py-2.5 rounded-xl bg-[#1147D9] text-white text-sm font-semibold hover:bg-[#1A55E6] transition-all">Quiz →</Link>
      </div>
    </div>
  )
}
