...
export default function PAQuizClient() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link href="/pa/quiz" className="flex items-center gap-2 text-sm text-[var(--on-surface-muted)] hover:text-[var(--on-surface)] mb-6 transition-colors"><ArrowLeft size={15} />Retour aux quiz</Link>
      <div className="flex items-center gap-3 mb-6">
        <span className="text-3xl">{mod.icon}</span>
        <div><h1 className="text-xl font-bold text-[var(--on-surface)]">{mod.title}</h1><p className="text-sm text-[var(--on-surface-muted)]">Quiz PA</p></div>
      </div>
      <QuizEngine track="pa" moduleId={mod.id} userId={user.id} tier={tier} />
    </div>
  )
}
