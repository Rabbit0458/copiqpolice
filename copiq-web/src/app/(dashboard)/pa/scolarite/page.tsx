...
export default function PAScolaritePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[var(--on-surface)]">PA — Scolarité</h1>
        <p className="text-[var(--on-surface-muted)] text-sm mt-1">Policier Adjoint · Cours préparatoires</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {PA_COURSE_MODULES.map((mod) => (
          <Link key={mod.id} href={`/pa/cours/${mod.id}`} className="rounded-2xl border border-[var(--outline)] bg-[var(--surface)] p-5 hover:border-[#1147D9]/40 hover:shadow-md transition-all group">
