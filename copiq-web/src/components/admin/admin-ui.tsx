        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/" className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold tracking-tight">
              COP&apos;IQ{" "}
              <span className="font-normal text-[var(--on-surface-faint)]">
                Admin
              </span>
            </span>
          </Link>
      ...
      <div className="flex items-start">
        ...
        >
          <nav aria-label="Navigation d'administration" className="space-y-5">
            {(["Pilotage", "Contenus", "Communauté", "Système"] as const).map(
              (group) => {
...
export function Stat({
  ...
  return (
    <Card className="p-4">
      ...
      <div
        className={`mt-1 text-2xl font-semibold tabular-nums ${colors[tone]}`}
      >
        {value}
      </div>
      {hint && (
