// COP'IQ — Version PA (Policier Adjoint) du module Culture générale.
// Copie adaptée de lib/content/gpx_exam/culture_generale/quiz_culture_generale_geographie.dart
...
// DATA MODEL
// ============================================================================
class PaQuizQuestion {
  final int id; // id unique en BDD
  final String module; // ex: "geography"
  final String category; // ex: "Geographie"
  ...
  factory PaQuizQuestion.fromJson(Map<String, dynamic> json) {
...
class PaQuizQuestionsRepository {
  final SupabaseClient sb;
  PaQuizQuestionsRepository(this.sb);
  ...
  Future<List<PaQuizQuestion>> fetchRandomSet({
    ...
    if (firstList.length >= limit) {
      return firstList.map(PaQuizQuestion.fromJson).toList();
    }
    ...
    return combined.map(PaQuizQuestion.fromJson).toList();
  }

  /// Fetch paginé (batch) — conservé si besoin plus tard.
  Future<List<PaQuizQuestion>> fetchBatch({
    required String category,
    String? difficulty,
...
// PAGE
// ============================================================================
class PaQuizCultureGeneraleGeographie extends StatefulWidget {
  static const String routeName =
      '/pa_exam/concours/culture_generale_geographie';

  final String uid;
  final String email;

  const PaQuizCultureGeneraleGeographie({
    super.key,
    required this.uid,
    required this.email,
  });
  ...
  @override
  State<PaQuizCultureGeneraleGeographie> createState() =>
      _PaQuizCultureGeneraleGeographieState();
}

class _PaQuizCultureGeneraleGeographieState
    extends State<PaQuizCultureGeneraleGeographie>
    with TickerProviderStateMixin {
  // ===========================================================================
  // CONFIG QUIZ
  // ===========================================================================
  static const String _categoryNameDb = 'Geographie';
  static const int _pageSize = 500;

  SupabaseClient get _sb => Supabase.instance.client;
  late final PaQuizQuestionsRepository _repo = PaQuizQuestionsRepository(_sb);

  late final PageController _page;
  late math.Random _rng;

  // Questions cache: index -> question
  final Map<int, PaQuizQuestion> _cache = {};
      ...
      for (int attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          if (startDb <= endDb) {
            ...
            final merged = <PaQuizQuestion>[...seg1, ...seg2];
  ...
  void _mapBatchToCache({
    ...
    required List<PaQuizQuestion> questions,
  }) {
    final shuffled = List<PaQuizQuestion>.from(questions);
  ...
  Future<void> _validate() async {
    ...
    unawaited(
      LearningAnswerHistoryService().record(
        ...
        quizKey: 'pa_quiz_culture_generale_geographie',
  ...
  Future<void> _createHistoryOnStart() async {
    ...
    try {
      ...
      final payload = <String, dynamic>{
        'uid': widget.uid,
        'email': widget.email,
        'module_name': 'PA - Culture générale',
        'quiz_name': 'PA - Quiz culture générale géographie',
  ...
  // `_updateHistoryOnAbandon` a été supprimée : elle écrivait un agrégat à
  // zéro, ce qui effaçait les réponses déjà validées quand l'utilisateur
  // fermait le quiz. Tous les chemins de sortie passent désormais par
  // `_updateHistoryOnFinish`, qui compte ce qui a été répondu. Le cas « fermé
  // sans aucune réponse » reste correct : l'agrégat vaut alors 0.
  ...
  Future<void> _saveAnswer({
    ...
    try {
      ...
      await _sb.from('quiz_culture_generale_geographie_pages').insert(payload);
    } catch (e, st) {
      debugPrint('❌ quiz_culture_generale_geographie_pages insert failed: $e');
      debugPrint('STACK: $st');
    }
  }
  ...
  PaQuizQuestion? get _currentQuestion => _cache[_index];

  Future<void> _insertReportCultureGenerale({
    required PaQuizQuestion q,
    ...
    final payload = <String, dynamic>{
      ...
      'message': message,
      'page': PaQuizCultureGeneraleGeographie.routeName,
      'status': 'new',
    };
        ...
        return AnnotatedRegion<SystemUiOverlayStyle>(
          ...
          child: Theme(
            ...
            child: PopScope<Object?>(
              // Le geste retour est renvoyé vers la même confirmation que la
              // croix : sans ça il la contournerait.
              canPop: !_hasQuiz,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // On masque les métadonnées / anciens labels (ex: "geographie_quiz_json")
          // qui n'ont rien à faire sous la widget.question.
          if ((widget.question.sub ?? '').trim().isNotEmpty &&
              (widget.question.sub ?? '').trim() != 'geographie_quiz_json') ...[
            const SizedBox(height: 6),
            Text(
              (widget.question.sub ?? '').trim(),
              style: TextStyle(
