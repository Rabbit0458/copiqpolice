...
// PAGE
// ============================================================================
class QuizFlagrantDelitPagePA extends StatefulWidget {
  static const String grade = 'pa';
  static const String routeName = '/pa/generalites/quiz/flagrant_delit';
  final String uid;
  final String email;

  const QuizFlagrantDelitPagePA({
    super.key,
    required this.uid,
    required this.email,
  });
  ...
  @override
  State<QuizFlagrantDelitPagePA> createState() =>
      _QuizFlagrantDelitPagePAState();
}

class _QuizFlagrantDelitPagePAState extends State<QuizFlagrantDelitPagePA>
  ...
  static const _introHiddenKey = 'intro_pa_flagrant_delit';
  ...
  // HELPERS
  // ==================================================================
  static const _questionModule =
      'pa_quiz_scolarite_pa_pa_quiz_flagrant_delit_page';

  Future<List<QuizQuestion>> _loadPublishedQuestions() async {
    var request = _sb
        .from('quiz_scolarite_questions')
        .select(
          'category,difficulty,question,options,answer,explanation,legal_ref',
        )
        .eq('module', _questionModule)
        .eq('is_active', true)
        .eq('publication_status', 'published');
    if (!_mixMode && _selectedDifficulty != null) {
      request = request.eq('difficulty', _selectedDifficulty!);
    }
    final rows = await request.order('position');
    return (rows as List)
        .whereType<Map<String, dynamic>>()
        .map((row) {
          final rawOptions = row['options'];
          return QuizQuestion(
            category: (row['category'] as String?) ?? 'Flagrant délit',
            ...
            difficulty: (row['difficulty'] as String?) ?? 'Moyenne',
            sub: row['legal_ref'] as String?,
          );
        })
        .where(
          (question) =>
  ...
  Future<void> _seedAndShuffle() async {
    final useAll = _mixMode || _selectedDifficulty == null;
    List<QuizQuestion> pool;
    try {
      pool = await _loadPublishedQuestions();
    } catch (error) {
      debugPrint(
        'quiz flagrant délit PA: chargement Supabase impossible — $error',
      );
      pool = const <QuizQuestion>[];
    }
    if (pool.isEmpty) {
      pool = useAll
  ...
  Future<void> _validate() async {
    ...
    unawaited(
      LearningAnswerHistoryService().record(
        ...
        quizKey: 'pa_quiz_flagrant_delit_page',
  ...
  Future<void> _insertReport({
    ...
    await _sb.from('report_question').insert(<String, dynamic>{
      ...
      'source_file': 'pa_quiz_flagrant_delit_page',
                          ...
                          children: [
                            ...
                            if (_showIntro)
                              _IntroSplash(
                                ...
                                description:
                                    'Maîtrise le régime du flagrant délit : définition, conditions, pouvoirs renforcés des enquêteurs et actes autorisés en flagrance.',
