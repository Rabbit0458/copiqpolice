...
class QuizFlagrantDelitPage extends StatefulWidget {
  ...
  @override
  State<QuizFlagrantDelitPage> createState() => _QuizFlagrantDelitPageState();
}

class _QuizFlagrantDelitPageState extends State<QuizFlagrantDelitPage>
  ...
  static const _questionModule =
      'gpx_dps_dpg_cadres_juridiques_pages_quiz_cadres_juridiques_quiz_flagrant_delit_page';
  ...
  Future<void> _seedAndShuffle() async {
    ...
    try {
      ...
      debugPrint(
        'quiz flagrant délit: chargement Supabase impossible — $error',
      );
      pool = const <QuizQuestion>[];
    }
    // Le contenu embarqué reste un filet de sécurité hors connexion. Dès que
    // Supabase répond, il est l'unique source des questions affichées.
    if (pool.isEmpty) {
      pool = useAll
  ...
  Future<void> _endQuizNow() async {
    if (_sortieEnCours || !_hasQuiz) return;
    _sortieEnCours = true;
    try {
      ...
      await _updateHistoryOnFinish();
      if (!mounted) return;
      final answered = _answers.whereType<String>().length;
      _openResultDialog(_score, answered == 0 ? 1 : answered);
    } finally {
