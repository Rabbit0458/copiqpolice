...
class GpxCasPratiqueCase4Page extends StatefulWidget {
  const GpxCasPratiqueCase4Page({super.key});

  static const String routeName = '/gpx_exam/concours/cas_pratique/case_4';

  @override
  State<GpxCasPratiqueCase4Page> createState() =>
      _GpxCasPratiqueCase4PageState();
}

class _GpxCasPratiqueCase4PageState extends State<GpxCasPratiqueCase4Page> {
  static const Color _kBlue = Color(0xFF1147D9);
  static const String _caseId = 'case_4';
  ...
  Future<void> _loadCorrection() async {
    ...
    try {
      ...
      debugPrint('[CASE_4_CORR][load_error] $e');
      if (!mounted) return;
      setState(() {
        _corrLoading = false;
        _corrError =
  ...
  // ─────────────────────────────────────────────
  // DONNÉES : réponses parfaites + rubrics (CAS 4)
  // ─────────────────────────────────────────────
  List<_PerfectAnswer> _buildPerfectAnswers() {
    // Q1 : opérations sur suspicion coup de feu / suicide
    const q1Perfect =
    ...
    // Q2 : annoncer décès + attitude face aux parents présents
    const q2Perfect =
    ...
    final q1Rubric = <_ExpectedPoint>[
      _ExpectedPoint(
        label:
            "Rendre compte CIC (TN00) + demander renforts / confirmer secours",
        groups: [
      ...
      _ExpectedPoint(
        label:
            "Sécurisation : périmètre / contrôle des accès / sécurité équipage",
        groups: [
      ...
      _ExpectedPoint(
        label:
            "Prise de contact + justification assistance personne en danger / ouverture (pompiers)",
        groups: [
          [
            'annonce',
            'police',
      ...
      _ExpectedPoint(
        label:
            "Si entrée : sécuriser arme + premiers secours / prise en charge",
        groups: [
          [
            'arme',
            'securiser',
      ...
      _ExpectedPoint(
        label:
            "Si décès/mort violente : préserver scène + compte rendu OPJ + témoins",
        groups: [
    ...
    final q2Rubric = <_ExpectedPoint>[
      _ExpectedPoint(
        label:
            "Vérifier identité des parents + les isoler dans un endroit calme",
        groups: [
          ['verifier', 'vérifier', 'identite', 'identité', 'piece', 'pièce'],
          ['ecart', 'à l ecart', 'calme', 'isoler', 'discret'],
        ],
      ),
      _ExpectedPoint(
        label:
            "Annonce du décès avec tact + sans détails inutiles/morbides + posture empathique",
        groups: [
          ['annoncer', 'annonce', 'deces', 'décès', 'decede', 'décédé'],
          ['tact', 'respect', 'empath', 'calme', 'bienveillance'],
        ],
      ),
      _ExpectedPoint(
        label:
            "Gérer leurs réactions + proposer assistance (eau, s’asseoir, secours si malaise)",
        groups: [
      ...
      _ExpectedPoint(
        label:
            "Préserver la scène : empêcher l’accès/ montée + rendre compte OPJ",
        groups: [
          [
            'ne pas monter',
            'interdire',
      ...
      _ExpectedPoint(
        label:
            "Informer sur suites (OPJ, démarches) + orientation/accompagnement",
        groups: [
          [
            'suite',
            'demarche',
                    ...
                    children: [
                      ...
                      Expanded(
                        child: PageView(
                          ...
                          children: [
                            const _IntroSlide(
                              title: "Cas pratique n°4",
                              subtitle:
                                  "Suspicion coup de feu / suicide + annonce de décès aux proches.",
                            ),
...
const String _caseText =
    ...
    "Elle vous précise que son voisin vit seul depuis que son épouse l'a quitté il y a un an et qu'il est dépressif. "
    "Elle vous informe également avoir contacté les pompiers et qu'elle s'apprêtait à appeler la police lorsqu'elle a vu votre véhicule.\n\n"
    "Elle vous précise que Monsieur X a fait une tentative de suicide médicamenteuse il y a trois mois environ.\n\n"
    "Monsieur X s'est suicidé par arme à feu : c'est une mort violente constatée par le médecin. Par ailleurs, une lettre a été retrouvée à proximité du corps dans laquelle Monsieur X explique les raisons de son geste.\n\n"
    "Le capitaine de police, officier de police judiciaire de permanence, a trouvé les coordonnées des parents de Monsieur X (adresse et numéro de téléphone). Ils demeurent à Xville. "
    "Le capitaine de police vous demande d'aviser les parents du décès de leur fils. Il poursuit ses constatations sur les lieux.\n\n"
    "Alors que vous vous apprêtez à quitter les lieux afin de mener à bien cette mission, un homme et une femme se présentant comme les parents de Monsieur X se trouvent au pied de l'immeuble et vous demandent ce qu'il se passe. "
    "Ils vous informent avoir tenté de joindre leur fils mais comme il ne répondait pas ils se sont déplacés.";
